...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && \Core\Security::verifyCsrf()) {
    ...
    if ($sessionUser) {
        ...
        if (isset($_POST['step1_submit'])) {
            ...
            $result = \Services\TransferService::validateTransfer($internetId, $amount, $_POST['payment_account'] ?? 'checking');
            if (!$result['ok']) {
                \Core\Session::flash('transfer_error', $result['error']);
            } else {
                \Core\Session::set('domestic_data', [
                ...
                \Core\Session::set('domestic_step', 2);
            }
            header('Location: ' . $_SERVER['REQUEST_URI']);
            exit;
        }
        if (isset($_POST['back_to_form'])) {
            \Core\Session::set('domestic_step', 1);
            header('Location: ' . $_SERVER['REQUEST_URI']);
            exit;
        }
        if (isset($_POST['confirm_submit'])) {
            \Core\Session::set('domestic_step', 3);
            \Core\Session::set('pending_transfer_type', 'domestic');
            header('Location: ' . $_SERVER['REQUEST_URI']);
            exit;
        }
        ...
        if (isset($_POST['pin_submit'])) {
            ...
            $data = \Core\Session::get('domestic_data');
            $result = ['ok' => false, 'error' => 'No pending transfer'];
            if ($data) {
                $data['internet_id'] = $postUser['internet_id'];
                $result = \Services\TransferService::domesticTransfer($data);
            }
            ...
            if ($result['ok']) {
                ...
                \Core\Session::set('domestic_data', null);
                \Core\Session::set('domestic_step', null);
...
$pageTitle = 'Domestic Transfer';
require_once __DIR__ . '/layout/header.php';

$data = \Core\Session::get('domestic_data') ?? [];
$step = (int) (\Core\Session::get('domestic_step') ?? 1);
...
<div class="mb-6 max-w-lg mx-auto">
    <h1 class="text-xl font-medium tracking-tighter text-gray-900 dark:text-white mb-4">Domestic Transfer</h1>
    ...
    <form method="POST">
        ...
        <div class="space-y-4">
            <div>
                ...
                <input type="text" name="recipient_name" value="<?= htmlspecialchars($data['recipient_name'] ?? '') ?>" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1e0e62]">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Account Number</label>
                <input type="text" name="recipient_account" value="<?= htmlspecialchars($data['recipient_account'] ?? '') ?>" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1e0e62]">
            </div>
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Bank Country</label>
                    <input type="text" name="bank_country" value="<?= htmlspecialchars($data['bank_country'] ?? '') ?>" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1e0e62]">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Account Type</label>
                    <select name="account_type" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1e0e62]">
            ...
            <div>
                ...
                <input type="number" name="amount" step="0.01" min="1" value="<?= htmlspecialchars((string) ($data['amount'] ?? '')) ?>" required class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1e0e62]">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Description</label>
                <input type="text" name="description" value="<?= htmlspecialchars($data['description'] ?? '') ?>" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1e0e62]">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Pay From</label>
                <select name="payment_account" class="w-full px-4 py-2.5 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-white/5 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-[#1e0e62]">
...
<div class="bg-white dark:bg-white/5 rounded-3xl shadow-sm p-6 max-w-lg mx-auto">
    ...
    <div class="divide-y divide-gray-100 dark:divide-white/10 text-sm">
        ...
        $rows = [
            ...
            'Account Type' => ucfirst($data['account_type'] ?? 'checking'),
